...
searchRouter.post('/search', async (req, res, next) => {
    try {
        ...
        if (companyRecords.length > 0) {
            ...
            if (insertError) {
                logger.error({ err: insertError, searchId: searchInsert.id }, 'Failed to persist companies');
            }
            else {
                const insertedRowsArray = (insertedRows ?? []);
                const insertedCount = Array.isArray(insertedRowsArray) && insertedRowsArray.length > 0
                    ? insertedRowsArray.length
                    : companyRecords.length;
                logger.info({ searchId: searchInsert.id, insertedCount }, 'Persisted companies for search');
            }
        }
