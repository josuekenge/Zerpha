...
const optionalStringField = (fallback) => z.preprocess((value) => (typeof value === 'string' ? value.trim() : undefined), z.string().min(1).catch(fallback));
...
async function requestExtraction(prompt) {
...
async function requestJsonCorrection(invalidJson, errorMessage) {
...
export async function extractCompanyInsights(input) {
...
async function parseWithRetry(rawJson, retryFn) {
    try {
        ...
        try {
            ...
            throw new Error(`Failed to parse Claude extraction response after retry: ${secondError.message}`);
        }
    }
}
