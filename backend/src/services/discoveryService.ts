...
Your task is to extract exactly 5 real companies that are directly relevant to the user's search query.

Rules:
1. Return exactly 5 companies, no more and no less.
...
8. All 5 companies must match the search intent clearly.
...
- The list contains exactly 5 companies.
...
export async function discoverCompanies(
  ...
  const message = await anthropic.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `Find exactly 5 real companies for this search query: "${query}"`,
      },
    ],
  });
